      ...
      <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-md z-50">
        <div className={`${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'} rounded-2xl shadow-2xl border overflow-hidden`}>
          ...
          <div className={`p-4 border-b ${darkMode ? 'border-slate-700' : 'border-gray-200'}`}>
            ...
            <p className={`mt-2 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'} line-clamp-2`}>
              {shareTitle}
            </p>
          </div>
          ...
          <div className="p-4">
            <div className="grid grid-cols-3 gap-3">
              {shareOptions.map((option) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 p-4 rounded-xl ${darkMode ? 'bg-slate-800 hover:bg-slate-750' : 'bg-gray-50 hover:bg-gray-100'} transition-all group`}
                >
                  <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${option.color} flex items-center justify-center text-white group-hover:scale-110 transition-transform`}>
                    {option.icon}
                  </div>
            ...
            {/* Copy Link */}
            <div className={`mt-4 p-3 rounded-xl ${darkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={shareUrl}
                  className={`flex-1 bg-transparent ${darkMode ? 'text-gray-300' : 'text-gray-700'} text-sm truncate focus:outline-none`}
                />
                <button
